...
struct OpenSourceKnowledgeServiceNetworkTests {
    ...
    func openSourceKnowledgeServiceTruncatesAndCleansUserQueryInGroqPayload() async throws {
            ...
            if url.absoluteString.contains("api.groq.com/openai/v1/chat/completions") {
                if let body = request.httpBody,
                   ...
                   let userMessage = messages.last?["content"] as? String {
                    stateQueue.sync { capturedUserMessage = userMessage }
                }
                let groqPayload: [String: Any] = [
                    "choices": [
                        [
        ...
        let userMessage = stateQueue.sync { capturedUserMessage ?? "" }
        let queryLine = userMessage
            .split(separator: "\n")
            .first(where: { $0.contains("Consulta del usuario:") })
            .map(String.init) ?? ""
        let extractedQuery = queryLine.replacingOccurrences(of: "Consulta del usuario: ", with: "")

        #expect(!extractedQuery.hasPrefix(" "))
        #expect(!extractedQuery.hasSuffix(" "))
        #expect(extractedQuery.count == 700)
    }
